...
in1 = distortimage.MetaMosaic(
      ...
      bbox=[12,24,12,24], extpix=800, verbose=True
      )
...
# get the center
ra_, dec_ = in1.wcs.all_pix2world(in1.Nside/2-.5, in1.Nside/2-.5, 0)

# mask a 3x3 grid of "stars" -- just for demo
ra_mask, dec_mask = np.meshgrid(ra_ + np.linspace(-1,1,3)/120., dec_ + np.linspace(-1,1,3)/120.)
ra_mask = ra_mask.ravel()
dec_mask = dec_mask.ravel()
radius_mask = 5./3600.
print(ra_mask, dec_mask, radius_mask); sys.stdout.flush()
in1.mask_caps(ra_mask, dec_mask, radius_mask)

# and we'll do one off to the side for good measure
dec_mask += 4./120.
radius_mask = np.linspace(1.,3.,len(ra_mask))/3600.
print(ra_mask, dec_mask, radius_mask); sys.stdout.flush()
in1.mask_caps(ra_mask, dec_mask, radius_mask)

in1.to_file('test-ex.fits') # write the block + extension
print('&&', time.time()-t0); sys.stdout.flush()
...
I_shear = in1.shearimage(2800, jac=[[0.97,0], [0,1.03]], psfgrow=1.06, oversamp=1.,
          Rsearch=3.5, select_layers=[0,4,6], verbose=True)
