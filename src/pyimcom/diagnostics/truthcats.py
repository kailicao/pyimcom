...
in1 = sys.argv[2]
outstem = sys.argv[3]
outfile_g = outstem + f"_TruthCat_{filter:s}.fits"
...
for iblock in range(nstart, nblockmax**2):
    ...
    infile = in1 + f"{filter:s}_{ibx:02d}_{iby:02d}_map.fits"
    ...
    for layerName in use_layers:
        ...
        if "gsext" in layerName:
            ...
            resolutionTables[this_res]["sersic_r_" + layerName] = truthcat["sersic"]["r"][
                :
