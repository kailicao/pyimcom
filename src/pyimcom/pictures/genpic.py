...
def make_picture_1band(
    fn, outfile, layer=0, bounds=None, binning=1, cmap=None, srange=(-8.0, 600.0), stretch="asinh"
):
    """
    ...
    if nint % binning > 0:
        raise Exception(f"genpic.make_picture_1band: can't bin {nint:d} in groups of {binning:d}")

    # and the subregion
    ymin, ymax, xmin, xmax = resolve_bounds(bounds, cfg.nblock)
    cube = np.zeros(
        ((ymax - ymin) * nint // binning, (xmax - xmin) * nint // binning, (1 if bw else 3)), dtype=np.uint8
    )
        ...
        for iy in range(ymax - ymin):
            ...
            if os.path.exists(fname):
                with fits.open(fname) as f:
                    ...
                    if bw:
                        cube[
                            iy * nint // binning : (iy + 1) * nint // binning,
                            ix * nint // binning : (ix + 1) * nint // binning,
                            0,
                        ] = cmapscale(D, srange, cmap=cmap, stretch=stretch)
                    else:
                        cube[
                            iy * nint // binning : (iy + 1) * nint // binning,
                            ix * nint // binning : (ix + 1) * nint // binning,
